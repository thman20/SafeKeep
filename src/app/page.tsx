...
export default function NewsletterManager() {
  ...
  const toggleHighValue = (id: string) => {
    setFlashingRow(id);
    setTimeout(() => setFlashingRow(null), 300);
    setSenders(prev => prev.map(s => s.id === id ? { ...s, isHighValue: !s.isHighValue } : s));
  };
        ...
        <AnimatePresence initial={false}>
          {filteredSenders.map(sender => (
            <motion.div 
              ...
              animate={{
                backgroundColor: flashingRow === sender.id ? '#FFD700' : '#ffffff',
                transition: { duration: flashingRow === sender.id ? 0.1 : 0.3 }
              }}
            ...
            >
              ...
              <div className="flex items-center gap-6 shrink-0 relative z-10">
                <motion.button 
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={() => toggleHighValue(sender.id)}
                  className={`p-2 rounded-full transition-colors ${sender.isHighValue ? 'text-primary-dark bg-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface'}`}
                  title="Mark as High-Value"
                >
                  <motion.svg animate={sender.isHighValue ? { scale: [1, 1.2, 1] } : {}} transition={{ duration: 0.3 }} className="w-6 h-6" fill={sender.isHighValue ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
